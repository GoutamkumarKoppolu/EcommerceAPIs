...
export const getSupplier = async (request:Request, response:Response)=>{
    ...
    //Pagination method
    if (offset && next){
        let a = await (await connect).query(`SELECT * FROM [ecommerceDb].[dbo].[Supplier] ORDER BY Id OFFSET ${offset} ROWS FETCH NEXT ${next} ROWS ONLY`)
        return response.status(200).json({message:"fetched requested data successfully", response:a})
    }
    else if(fieldname && columnname){
        let filteredData = await (await connect).query(`SELECT * FROM [ecommerceDb].[dbo].[Supplier] WHERE [${columnname}] LIKE '${fieldname}'`)
    ...
    //get all method
        ...
        let a = await (await connect).query(`SELECT * FROM [ecommerceDb].[dbo].[Supplier]`)
        return response.status(200).json({message:"fetched requested data successfully", response:a})
    }
    else if (!offset || !next){
...
export const getOneSupplier = async(request:Request, response:Response) =>{
    ...
    let a = await (await connect).query(`select * from [ecommerceDb].[dbo].[Supplier] where Id =${id}`)
    if (a[0]){
...
export const insertIntoSupplier = async (request: Request, response: Response)=>{
    const newSupplier = {
        ...
        if(newSupplier.CompanyName){
            let a = await (await connect).query(`insert into [ecommerceDb].[dbo].[Supplier] ([CompanyName], [ContactName], 
            ...
            return response.send("Please enter the company name which is mandatory!")
        }
...
export const updateAsupplier = async(request:Request, response:Response)=>{
    let id = request.params.id;
    let result = await(await connect).query(`select top (1) * from [eCommerceDb].[dbo].[Supplier] where Id=${id}`);
    if (!result[0]){
        ...
        let query_response = await(await connect).query(`update [eCommerceDb].[dbo].[Supplier] set CompanyName='${CompanyName}',
        ContactName='${ContactName}',
        ContactTitle='${ContactTitle}',City='${City}',Country='${Country}',Phone='${Phone}',Fax='${Fax}'`);

        let updated_data = await(await connect).query(`select * from [eCommerceDb].[dbo].[Supplier] where id=${id}`);
...
export const deleteASupplier = async(request: Request, response: Response) =>{
    let id = request.params.id;
    let result = await (await connect).query(`select * from [ecommerceDb].[dbo].[Supplier] where Id=${id}`);
    if (result[0]){
        let deletedColumn = await (await connect).query(`DELETE FROM [ecommerceDb].[dbo].[Supplier] WHERE Id = ${id}`);
